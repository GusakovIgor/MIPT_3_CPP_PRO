...
private:
	...
	Node* FindElement (bool (*cmp_func) (Element*));

	Node* FindElement (ElemType type, int detailed_type, Element* elem);


	Node* FindElementInSubTree (Node* entry, bool (*cmp_func) (Element*));

	Node* FindElementInSubTree (Node* entry, ElemType type, int detailed_type);

	Node* FindElementInSubTree (Node* entry, Element* elem);
	...
	Functor* SetTurnPage 		 (Node* entry, EventManager* event_manager);
...
public:
	...
	Element* AddElement (EnabledType enabled, ElemConfig* config, Element* parent = nullptr);
