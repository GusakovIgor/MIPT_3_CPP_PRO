...
ElemTree::ElemTree (Application* App, const char* hierarchy_conf)
{
	...
	AddElement (EnabledType::enabled, main, nullptr);
}
...
Element* ElemTree::AddElement (EnabledType enabled, ElemType type, int detailed_type, Element* parent)
{
	...
	if (type == ElemType::window_t && detailed_type == canvas_t)
	{
		...
		result = AddElement (enabled, config, parent);


		Node* canvas_ptr_parent_node = FindElement (IsLastCanvasPage);

		Element* canvas_ptr_parent = canvas_ptr_parent_node->elem;


		CanvasPointerPage* canvas_ptr_page = dynamic_cast <CanvasPointerPage*> (canvas_ptr_parent);

		if (canvas_ptr_page->Full ())
		{
			canvas_ptr_parent = AddElement (EnabledType::enabled, ElemType::window_t, canvas_ptr_page_t, nullptr);
		}

		AddElement (EnabledType::enabled, ElemType::button_t, canvas_ptr_t, canvas_ptr_parent);

		canvas_ptr_page->num_canvas_pointers++;


		if (canvas_ptr_page->num_canvas_pointers == 1)
		{
			Pages* pages = dynamic_cast <Pages*> (canvas_ptr_parent_node->parent->elem);
...
Element* ElemTree::AddElement (EnabledType enabled, ElemConfig* config, Element* parent)
{
	assert (config);
	...
	Node* new_node = new Node (enabled, new_elem, parent_node);
	...
	for (ElemConfig* child : config->children)
	{
		...
		{
			AddElement (child->enabled, child, new_elem);
		}
	}
...
bool IsLastCanvasPage (Element* elem)
{
	if (elem->GetType () != ElemType::window_t || elem->GetDetailedType () != canvas_ptr_page_t)
	{
		return false;
	}

	CanvasPointerPage* target = dynamic_cast <CanvasPointerPage*> (elem);
...
Node* ElemTree::FindElement (bool (*cmp_func) (Element*))
{
	return FindElementInSubTree (head, cmp_func);
}
...
Node* ElemTree::FindElementInSubTree (Node* entry, bool (*cmp_func) (Element*))
{
	if (cmp_func (entry->elem))
	{
		return entry;
	}
	...
	{
		target = FindElementInSubTree (child, cmp_func);
...
bool ElemTree::ApplyClickForSubTree (Node* entry, Mouse* mouse)
{
	...
	{
		if (entry->elem->GetType () == ElemType::window_t && entry->elem->GetDetailedType () == pages_t)
		{
			Pages* pages = dynamic_cast <Pages*> (entry->elem);

			int active = pages->active;

			auto it = entry->children.begin ();

			for (int i = 0; i < active && it != entry->children.end (); ++i)
			{
				++it;
			}

			if (ApplyClickForSubTree (*it, mouse))
			{
				return true;
			}
		}
		...
		{
			if (mouse->LeftClick () && entry->elem->GetDetailedType () != canvas_ptr_t)
			{
...
void ElemTree::SetFunctor (Node* entry, EventManager* event_manager)
{
	...
	switch (entry->elem->GetDetailedType ())
	{
		...
		case turn_page_t:			functor = SetTurnPage (entry, event_manager);
...
Functor* ElemTree::SetTurnPage (Node* entry, EventManager* event_manager)
{
	static int new_turn_page = 0;
	...
	Node* help_elem = FindElement (ElemType::window_t, pages_t, nullptr);

	Pages* pages = dynamic_cast <Pages*> (help_elem->elem);

	Functor* functor = new TurnPage (&(pages->active), delta, event_manager);
