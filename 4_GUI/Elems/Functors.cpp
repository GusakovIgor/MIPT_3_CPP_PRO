...
TurnPage::TurnPage (int* init_active, int init_delta, EventManager* init_manager) : Functor (init_manager)
{
	delta = init_delta;
...
void TurnPage::Act (Mouse* mouse)
{
	if (mouse->LeftClick ())
	{
		if (0 <= *active + delta && *active + delta <= CanvasPointerPage::num_pages)
		{
			*active += delta;
		}
	}
