...
void Application::CheckEvents (Mouse* mouse)
{
	...
	while (SDL_PollEvent (&event))
	{
		switch (event.type)
		{
			...
			case SDL_MOUSEBUTTONDOWN:	mouse->location.x = event.button.x;
										mouse->location.y = event.button.y;
										mouse->SetState (static_cast<MouseState> (event.button.button));
										return;


			case SDL_MOUSEBUTTONUP:		mouse->location.x = event.button.x;
										mouse->location.y = event.button.y;
										mouse->SetState (wrong_b);
										return;
